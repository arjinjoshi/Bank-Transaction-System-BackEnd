...
async function createTransaction(req, res) {
  ...
  if (!fromUserAccount || !toUserAccount) {
    return res.status(400).json({
      message: "Invlaid fromAccount or toAccount",
    });
  }
  ...
  /**
   * 4. Derive senderBalance from ledger
   */
  ...
  try {
     ...
     * 5. Create Transaction (PENDING)
     */
    const session = await mongoose.startSession(); //helps to maintain consistency so that it can revert back if anything goes wrong after this
